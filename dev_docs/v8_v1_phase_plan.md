...
9. Continue residual clustering: trusted external import helpers. Bitfield,
   partial-overwrite byte/bit precision, large-struct return-buffer flows, and
   nested deep-field pointer passthrough are covered by focused all-root gates.
   [in progress]
```
