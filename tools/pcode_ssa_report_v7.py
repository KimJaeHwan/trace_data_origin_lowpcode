...
from pcode_ssa_report_v6 import SourceSinkCallSiteBinderV6
...
class MemoryObjectTransferSSAEngineV7(SourceSinkCallSiteBinderV6):
    """
    v7 differences from pcode_ssa_report_v6.py:
    - Keeps v6 source/sink binding and generated helper summary DB support.
    - Adds recursive stack address recovery for nested Low-PCode address expressions
      such as [EBP + ECX*scale + offset]. V6 often collapsed these to REG_0x7200.
    - Adds narrow library transfer summaries for memcpy, memmove, strcpy, and memset.
    - Starts treating stack memory as transfer targets between object-like stack slots,
      while leaving full byte-range/heap object precision for later work.
    """

    def __init__(self, json_path, summary_path=None):
        super().__init__(json_path)
        self.summary["notes"].append("v7 adds recursive stack-address recovery and narrow libc transfer summaries.")

    def _parse_signed_offset(self, value):
        if value is None:
            return None
    ...
    def _format_stack_offset(self, value):
        return "0x%08x" % (value & 0xffffffff)

    def _const_from_node(self, node, visited=None):
        if not node:
            return None
        if visited is None:
            visited = set()
        ...
        if node.startswith("Const_"):
            return self._parse_signed_offset(node[len("Const_"):])
        if not self.G.has_node(node):
            return None

        opcode = self.G.nodes[node].get("opcode")
        preds = list(self.G.predecessors(node))
        values = [self._const_from_node(pred, set(visited)) for pred in preds]

        if opcode == "LOAD":
            for pred in preds:
                if self.G.edges[pred, node].get("kind") == "memory":
                    value = self._const_from_node(pred, set(visited))
                    if value is not None:
                        return value
        if opcode in {"STORE_VAL", "CALL_LIBC_MEMSET"}:
            for pred in preds:
                if self.G.edges[pred, node].get("kind") == "data":
                    value = self._const_from_node(pred, set(visited))
                    if value is not None:
                        return value

        if opcode in {"COPY", "INT_SEXT", "INT_ZEXT", "SUBPIECE"} and values:
            return values[0]
        if opcode == "INT_ADD" and len(values) >= 2 and all(value is not None for value in values[:2]):
            return values[0] + values[1]
        if opcode == "INT_SUB" and len(values) >= 2 and all(value is not None for value in values[:2]):
            return values[0] - values[1]
        if opcode == "INT_MULT" and len(values) >= 2 and all(value is not None for value in values[:2]):
            return values[0] * values[1]
        if opcode == "INT_LEFT" and len(values) >= 2 and all(value is not None for value in values[:2]):
            return values[0] << values[1]
        if opcode == "INT_AND" and len(values) >= 2 and all(value is not None for value in values[:2]):
            return values[0] & values[1]
        if opcode == "INT_OR" and len(values) >= 2 and all(value is not None for value in values[:2]):
            return values[0] | values[1]
        return None

    def _stack_expr_from_node(self, node, visited=None):
        if not node:
            return None
        if visited is None:
            visited = set()
        ...
        const_value = self._const_from_node(node, set())
        if const_value is not None:
            return {"kind": "const", "value": const_value}
        if node.startswith("EBP#") or node.startswith("RBP#"):
            return {"kind": "stack", "base": "EBP", "offset": 0}
        if not self.G.has_node(node):
            return None

        opcode = self.G.nodes[node].get("opcode")
        preds = list(self.G.predecessors(node))
        if opcode in {"COPY", "INT_SEXT", "INT_ZEXT", "SUBPIECE"} and preds:
            return self._stack_expr_from_node(preds[0], set(visited))
        if opcode in {"INT_ADD", "PTRADD", "PTRSUB"} and len(preds) >= 2:
            exprs = [self._stack_expr_from_node(pred, set(visited)) for pred in preds[:2]]
            ...
            if stack_expr and const_expr:
                return {"kind": "stack", "base": stack_expr["base"], "offset": stack_expr["offset"] + const_expr["value"]}
            if stack_expr:
                return stack_expr
        return None

    def _resolve_stack_address(self, addr_node):
        expr = self._stack_expr_from_node(addr_node)
        if expr and expr.get("kind") == "stack":
            return "%s_%s" % (expr["base"], self._format_stack_offset(expr["offset"]))
        return super()._resolve_stack_address(addr_node)

    def _stack_arg_key(self, args, index):
        if index is None or index >= len(args):
            return None
        return self._resolve_stack_key_from_node(args[index])

    def _apply_libc_transfer_summary(self, instr, state, target_name):
        if target_name not in {"memcpy", "memmove", "strcpy", "memset"}:
            return False
        addr = instr["address"]
        args = list(self.pending_stack_args)
        dest_key = self._stack_arg_key(args, 0)
        if not dest_key:
            return False

        if target_name in {"memcpy", "memmove", "strcpy"}:
            src_key = self._stack_arg_key(args, 1)
            if not src_key:
                return False
            src_node = state["stack"].get(src_key)
            if not src_node:
                return False
            mem_node = self._create_new_version("Memory", dest_key, state)
            self.G.add_node(mem_node, addr=addr, opcode="CALL_LIBC_COPY", target=target_name)
            self._add_dependency(src_node, mem_node, "memory")
            state["stack"][dest_key] = mem_node
            self.stack_memory_map[dest_key] = mem_node
            self.ssa_pcode_lines.append(f"{mem_node} = CALL_LIBC_COPY_SUMMARY({target_name}, {dest_key} <- {src_key}, {src_node})")
            self.callsite_bindings.append({
                "addr": addr,
                "kind": "libc_transfer_summary_v7",
                "target": target_name,
                "dest_stack_key": dest_key,
                "src_stack_key": src_key,
                "source_node": src_node,
                "confidence": "resolved_direct_call_libc_transfer"
            })
        else:
            value_node = args[1] if len(args) > 1 else None
            if not value_node:
                return False
            mem_node = self._create_new_version("Memory", dest_key, state)
            self.G.add_node(mem_node, addr=addr, opcode="CALL_LIBC_MEMSET", target=target_name)
            if self.G.has_node(value_node):
                self._add_dependency(value_node, mem_node, "data")
            state["stack"][dest_key] = mem_node
            self.stack_memory_map[dest_key] = mem_node
            self.ssa_pcode_lines.append(f"{mem_node} = CALL_LIBC_MEMSET_SUMMARY({target_name}, {dest_key}, {value_node})")
            self.callsite_bindings.append({
                "addr": addr,
                "kind": "libc_memset_summary_v7",
                "target": target_name,
                "dest_stack_key": dest_key,
                "value": value_node,
                "confidence": "resolved_direct_call_libc_transfer"
        ...
        self.summary["callsites"].append({
            ...
            "target": target_name,
            "model": "libc_transfer_summary_v7",
            "args": args,
        })
    ...
    def _process_instruction(self, instr, state):
        return super()._process_instruction(instr, state)
    ...
    def generate_report(self, target_node, output_path, expected_path=DEFAULT_EXPECTED_PATH):
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("==================================================\n")
            f.write(f"V7 MEMORY OBJECT TRANSFER REPORT: {self.data.get('function_name')}\n")
            f.write("==================================================\n\n")

            f.write(" V7 notes\n")
            f.write("--------------------------------------------------\n")
            f.write("- v6 source/sink binding and generated helper summary DB behavior is preserved.\n")
            f.write("- v7 recovers nested EBP-relative stack addresses before memory linking.\n")
            f.write("- v7 models memcpy, memmove, strcpy, and memset as narrow stack transfer summaries.\n")
            f.write("- Full heap objects, byte ranges, and indirect-call resolution remain future work.\n\n")
        ...
        print(f"[+] V7 report generated: {output_path}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(json_input):
        engine = MemoryObjectTransferSSAEngineV7(json_input)
